...
| 5 (Visual Recon) | `57b6ca4` | `ghost_recon_frames` table (vector(1536)) + upsert/search/count SECURITY DEFINER RPCs, multimodal `images` array on gatewayChatText/Json, 12-frame YouTube thumbnail-ladder sampler (no ffmpeg, Edge-safe), multimodal Flash captioning with batched text-embedding-3-small vectors, `/api/ghost/recon-ingest` + `/api/ghost/recon-search` edge routes, per-user useReconStore, GhostVisualRecon BLACK-OPS panel with clickable ?t= deep-links, 3 new tests | ✅ shipped |
| 6 (Dawn Patrol) | `3d84a64` | `ghost_dawn_patrol_briefs` table + SECURITY DEFINER upsert/latest/mark_read/config_get/config_set/due_users RPCs, `referral_profiles.dawn_patrol_enabled`+`dawn_patrol_send_hour` preferences, pg_cron hourly dispatch (best-effort) via pg_net to `/api/ghost/dawn-patrol-cron` (CRON_SECRET-gated), 3-bullet sunrise brief generated by gatewayChatJson with deterministic fallback, email delivery reserved (skipped in MVP until provider key present), `/api/ghost/dawn-patrol-generate/latest/mark-read/config/cron` edge routes, per-user useDawnPatrolStore + useDawnPatrol React Query hook (5m poll, focus refetch, toast on new brief), DawnPatrolCard (sunrise amber theme, 3 bullets, opportunities/threats tags, inline preferences panel), mounted on CloneCrush above Squad Dossier, 4 new tests | ✅ ready |
| 7 (Perf pass) | — | **Bug stomp:** unified `api/_ghostAuth.ts` identity layer replaces five drifted per-module `verifyAuth()` copies that validated the caller's JWT using the *service-role key* as the Authorization bearer — GoTrue resolved the service identity, so any non-empty bearer was accepted and `p_user_id` no longer tracked the caller (auth bypass + cross-user leakage across interrogate/squad/recon/dawn-patrol/ledger). **P95 pass:** short-TTL memoisation collapses the route+`consumeGhostAction` double GoTrue verification into one round-trip; negative caching blunts token-spray amplification; 5xx never cached; interrogate chat now issues auth and query-embed concurrently (removes one serialised RTT from every turn). **Bundle audit:** recharts hoisted out of the CompetitorShowdown route chunk into a cache-stable `charts` vendor chunk — route chunk 424.00 kB → 15.74 kB (−96%), library no longer re-downloaded on app-code deploys. **Docs:** README Appendix A (Ghost Intel Modules, text-only enterprise grade). 14 new auth regression tests; 507 total green. | ✅ shipped |
