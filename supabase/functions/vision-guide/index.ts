...
function toInlineData(imageData: string) {
  ...
  if (!matches) throw new Error("Invalid image format. Upload valid screenshots.");
  return {
    inlineData: {
      mimeType: matches[1],
      data: matches[2],
...
serve(async (req) => {
  ...
  try {
    ...
    if (!geminiApiKey) {
      return jsonResponse({ success: false, error: "GEMINI_API_KEY not configured on server.", action: "Admin: set env." }, 500);
    }
    ...
    if (images.length > 10) {
      return jsonResponse({ success: false, error: "Too many images. Max 10.", action: "Remove some images." }, 400);
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        return jsonResponse({ success: false, error: "Gemini rate limit exceeded.", action: "Wait 30 seconds." }, 429);
      }
