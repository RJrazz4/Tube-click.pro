...
serve(async (req) => {
  ...
  try {
    ...
    if (!FAL_API_KEY) {
      return new Response(
        JSON.stringify({ success: false, error: 'FAL_API_KEY not configured on server.', action: 'Admin: set env.' }),
    ...
    if (!prompt) {
      return new Response(
        JSON.stringify({ success: false, error: 'Prompt is required.', action: 'Enter description.' }),
    ...
    if (!response.ok) {
      if (response.status === 401 || response.status === 403) {
        return new Response(
          JSON.stringify({ success: false, error: 'Server Fal.ai key invalid.', action: 'Check FAL_API_KEY env.' }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      if (response.status === 429) {
        return new Response(
          JSON.stringify({ success: false, error: 'Fal.ai rate limit exceeded.', action: 'Wait a moment.' }),
          { status: 429, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      return new Response(
        JSON.stringify({ success: false, error: `Image API error: ${response.status}`, action: 'Try again.' }),
