...
export function ReferralMilestones({ profile }: { profile: ReferralProfile }) {
  const milestones = [
    ...
    { label: `Complete ${profile.requiredForReward} qualified referrals`, detail: "A referral qualifies once your invited creator completes a real action \u2014 signing up is not enough.", complete: profile.qualifiedReferrals >= profile.requiredForReward },
    { label: `Unlock ${profile.rewardDays} days of Pro`, detail: "Granted automatically at the milestone. Expiry is shown in this dashboard.", complete: profile.proActive },
  ];
