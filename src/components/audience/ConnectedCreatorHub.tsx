...
import { Activity, CheckCircle2, Cable, Database, RefreshCw, ShieldCheck, Sparkles, Terminal, TrendingUp, Wifi } from "lucide-react";
...
export function ConnectedCreatorHub({ connection }: { connection: ReturnType<typeof useYouTubeConnection> }) {
  ...
  return (
    <div className="space-y-4 md:space-y-5 animate-fade-in">
      {/* ─── Header: authentic branding + live sync ─────────────────────── */}
      <Card className="glass-strong bracket border-primary/20 relative overflow-hidden">
        ...
        <CardContent className="relative flex flex-col gap-4 p-5 md:p-6 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-3 sm:flex-col sm:items-end sm:text-right">
            ...
            <div className="text-[10px] font-mono text-muted-foreground">
              <p className="flex items-center gap-1 justify-center sm:justify-end"><ShieldCheck className="h-3 w-3 text-emerald-500" /> {scopesGranted} read-only scopes</p>
        ...
        <>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Card className="cyber-card border-border/70">
              ...
              <CardContent>
                <ChartContainer config={momentumConfig} className="h-[240px] w-full">
              ...
              <CardContent>
                {geoTotal > 0 ? (
                  <div className="flex flex-col items-center gap-3 sm:flex-row">
                    <ChartContainer config={geoConfig} className="h-[220px] w-full max-w-[260px] mx-auto">
                      <PieChart accessibilityLayer>
                        <ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel />} />
                        <Pie data={geoShare} dataKey="value" nameKey="name" innerRadius={58} outerRadius={88} paddingAngle={3} strokeWidth={0}>
