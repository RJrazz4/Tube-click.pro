        ...
        <section aria-labelledby="library-heading" className="space-y-3">
          <div className="flex items-end justify-between gap-3">
            ...
            <Button asChild variant="outline" size="sm" className="shrink-0 gap-1.5 text-xs">
              <Link to="/library">View all <ArrowUpRight className="h-3.5 w-3.5" /></Link>
            </Button>
          </div>
          <div className="grid lg:grid-cols-2 gap-5 md:gap-6">
          <Card className="glass-strong border-border"><CardHeader className="pb-3"><CardTitle className="font-display text-base flex items-center gap-2"><Terminal className="w-4 h-4 text-primary" />Your Library</CardTitle><CardDescription className="text-sm text-muted-foreground flex items-center gap-1"><Cpu className="w-3 h-3" />{totalContent>0?`${totalContent} saved items • this browser`:"Nothing saved yet — pick a tool below to start!"}</CardDescription></CardHeader><CardContent>{recentContent.length>0 ? (<div className="space-y-3">{recentContent.map((content:any)=>{ const Icon = getContentIcon(content.type); return (<div key={content.id} className="group relative flex items-center gap-3 p-3 md:p-4 bg-secondary/50 backdrop-blur-sm rounded-xl border border-border/30 hover:border-primary/30 transition-all"><button onClick={()=>handleDeleteItem(content.id)} aria-label={`Delete ${content.title}`} title="Delete saved item" className="absolute -top-2 -right-2 z-10 w-7 h-7 rounded-full bg-secondary/90 border border-border flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-destructive/20 hover:border-destructive/50 transition-all opacity-0 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-destructive"><X className="w-3.5 h-3.5" /></button><div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0"><Icon className="w-5 h-5 text-primary" /></div><div className="flex-1 min-w-0"><p className="text-sm text-foreground truncate font-medium">{content.title}</p><p className="text-xs text-muted-foreground flex items-center gap-1.5 mt-0.5"><span className="capitalize">{content.type}</span><span>•</span><span>{new Date(content.createdAt).toLocaleDateString()}</span></p></div></div>);})}</div>) : (<div className="text-center py-8"><div className="w-16 h-16 mx-auto rounded-2xl bg-primary/10 flex items-center justify-center mb-4"><Sparkles className="w-8 h-8 text-primary animate-pulse" /></div><p className="text-base text-foreground font-medium mb-1">Your library is empty</p><p className="text-sm text-muted-foreground">Run any tool above — everything you save appears here.</p></div>)}</CardContent></Card>
